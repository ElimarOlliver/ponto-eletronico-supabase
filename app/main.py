...
def supabase_with_user(jwt: str) -> Client:
    ...
    # Validate token by fetching user (raises if invalid/expired)
    sb.auth.get_user(jwt)
    # Attach JWT for PostgREST so policies apply
    sb.postgrest.auth(jwt)  # relies on supabase-py/postgrest-py
    return sb
...
def my_punches(authorization: str | None = Header(default=None)):
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(status_code=401, detail="Missing Bearer token")
    jwt = authorization.split(" ", 1)[1]
    sb = supabase_with_user(jwt)
    data = sb.table("punches").select("*").order("occurred_at", desc=True).limit(20).execute().data
    return JSONResponse(data)

@app.post("/api/clock")
async def clock(request: Request, authorization: str | None = Header(default=None)):
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(status_code=401, detail="Missing Bearer token")
    jwt = authorization.split(" ", 1)[1]
